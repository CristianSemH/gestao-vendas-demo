...
import { getUsersById, addUser, updateUser } from '../../services/usuario';
...
const InputForm = () => {
    ...
    async function getDataUpdate(id, reset, token) {
        if (id) {
            const result = await getUsersById(id, token)
    ...
    const handleUpdateCreate = async (usuario) => {

        setMesssagesError([])

        if (id) {
            const result = await updateUser(id, usuario, token)
            ...
            const result = await addUser(usuario, token)
                        ...
                        <Form onSubmit={handleSubmit(handleUpdateCreate)}>
                            ...
                            <div className="col-12">
                                ...
                                <div className='float-end'>
                                    <button className='button-custom button-custom-secondary' onClick={() => navigate('/usuario')}> Voltar</button>
                                </div>
                            </div>
                        </Form>
