...
import { getTipoSituacaos, deleteTipoSituacao } from '../../services/tipoSituacaoVenda';
...
const TipoSituacaoList = () => {
    ...
    const handleDelete = async (tipoSituacaoId) => {
        await deleteTipoSituacao(tipoSituacaoId, token);
        if (currentPage === 1) {
    ...
    return (
        <div className="col-12">
            <HeaderList title="Lista de tipo situação venda" nav="Home / tipo situação venda"></HeaderList>
            <div className='card-custom shadow-sm'>
                <div className='card-custom-header mb-3 col-12'>
                    <ButtonNew eventOnClick={() => navigate('/tipoSituacaovenda/create')}></ButtonNew>
                        ...
                        <tbody>
                            {tipoSituacaos?.length > 0 && (tipoSituacaos.map((tipoSituacao) => (
                                <tr key={tipoSituacao.id}>
                                    <td>
                                        <ButtonAction eventDelete={() => handleDelete(tipoSituacao.id)} eventUpdate={() => navigate(`/tiposituacaovenda/update/${tipoSituacao.id}`)}></ButtonAction>
                                    </td>
